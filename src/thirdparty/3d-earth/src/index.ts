...
class Earth {
  ...
  autoRotateEarth() {
    ...
    var rotateEarthStep = new TWEEN.Tween({
      ...
      .to({ rotateY: endRotateY, zoom: endZoom }, 360000) //.to({rotateY: endRotateY, zoom: endZoom}, 10000)
    ...
    var rotateEarthStepBack = new TWEEN.Tween({
      ...
      .to({ rotateY: 3.15 * Math.PI * 2, zoom: startZoom }, 360000) //.to({rotateY: endRotateY, zoom: endZoom}, 10000)
      .easing(TWEEN.Easing.Quadratic.Out)
      .onUpdate(function (object: any) {
        if (that.earth3dObj) {
          that.earth3dObj.rotation.set(0, object.rotateY, 0);
  ...
  animate = () => {
    ...
    if (this.earth3dObj) {
      if (this.earthConfig.autoRotate && !this.earthConfig.zoomChina) {
        this.earth3dObj.rotation.y += 0.001;
      }
    }
